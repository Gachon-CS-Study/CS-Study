...
2. 정보를 주고 받을때 정보를 보내거나 받는다는 신호절차를 거치지 않는다. (데이터 수신여부를 확인하지 않는다.)
3. 신뢰성 없는 데이터를 전송한다. (데이터 재전송과 데이터 순서 유지를 위한 작업을 하지 않는다.)
4. 패킷 오버헤드가 적어 네트워크 부하가 감소되는 장점.
5. 상대적으로 TCP보다 전송속도가 빠르다.
